...
<div
	...
	<g:select name="origin"
		...
		value="${evaluationInstance?.origin}"
		valueMessagePrefix="evaluation.origin" />

</div>

<div
	class="fieldcontain ${hasErrors(bean: evaluationInstance, field: 'criterion', 'error')} required">
	<label for="criterion"> <g:message
			code="evaluation.criterion.label" default="Criterion" /> <span
		class="required-indicator">*</span>
	</label>
	<g:select id="criterion" name="criterion.id"
		from="${ta.Criterion.list()}" optionKey="id" required=""
		optionValue="${{it.description}}"
		value="${evaluationInstance?.criterion?.id}" class="many-to-one" />
		...
		<thead>
			<tr>
				...
				<g:sortableColumn property="name"
					title="${message(code: 'evaluation.origin.label', default: 'Name')}" />
		...
		<tbody>
			<g:each in="${ta.Student.list()}" status="i" var="studentInstance">
				<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					...
					<td><g:link action="show" id="${studentInstance.id}">
					...
					<td>
						...
						<div
							...
							<label for="value"> <g:message
									code="evaluation.value.label" default="Value" /> <span
								class="required-indicator">*</span>
							</label>
							<g:select name="value"
								from="${evaluationInstance.constraints.value.inList}"
								required="" value="${evaluationInstance?.value}"
